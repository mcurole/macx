function Import-Excel
{
  ...
  for ($column = 1; $column -le $columns; $column ++) {
    $fieldName = $sheet.Cells.Item(1, $column).Text
    if ($fieldName -eq "") {
...
#    $values = New-Object object[] $columns
    $values = @{}
    for ($column = 1; $column -le $columns; $column++) {
#      $values[$column - 1] = $sheet.Cells.Item($line, $column).Text
        $values.Add($fields[$column - 1], $sheet.Cells.Item($line, $column).Text)
    }  
  
#    $row = New-Object psobject
     $row = New-Object psobject -Property $values
...
Set-Alias -Name ipxls -Value Import-Excel

Export-ModuleMember -Function Import-Excel
Export-ModuleMember -Alias ipxls